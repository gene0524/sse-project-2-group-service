...
def add_group_to_groupbase(group_name, description):
    data_to_insert = {"group_name": group_name, "description": description}
    data, _ = (
        supabase_client.table("Group Registration").insert([data_to_insert]).execute()
    )
    return data[1]
...
def add_member_to_group(group_id, email, status):
    data_to_insert = {"group_id": group_id, "email": email, "status": status}
...
def display_user_groups(user_email):
    response, _ = (
        supabase_client.table("Group Members Info")
...
def remove_group(group_id, email):
    ...
    if member_status == 2:
        ...
        remove_member_from_group(
            group_id, email
        )  # If user is member, remove the user from the group
        return True
    else:
...
def display_top_votes(group_id):
    voted_food, _ = (
        supabase_client.table("Group Food List")
        .select("dish_uri")
        .eq("group_id", group_id)
        .gt("votes_count", 0)
...
def display_vote_options(group_id, user_email):
    ...
    response_2, _ = (
        supabase_client.table("Group Vote")
        .select("dish_uri")
        .eq("group_id", group_id)
        .eq("email", user_email)
        .execute()
...
def click_vote_dish(group_id, user_email, dish_uri):
    ...
    if valid_click:
        ...
        data, _ = supabase_client.table("Group Vote").insert([data_to_insert]).execute()
...
def add_food_to_food_list(group_id, dish_uri):
    ...
    data_to_insert = {"group_id": group_id, "dish_uri": dish_uri, "votes_count": 0}
    data, error = (
        supabase_client.table("Group Food List").insert([data_to_insert]).execute()
    )
    if not data:
